<x-app-layout>
    <div class="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow mt-8">
                ...
                <div class="flex items-start">
                    ...
                    @if(Str::contains(strtolower($label), 'alamat'))
                        <textarea rows="2" readonly
                            class="flex-1 border border-gray-300 rounded-md p-2 text-sm bg-gray-50">{{ $value }}</textarea>
                    @else
                        <input type="text" readonly value="{{ $value }}"
                            class="flex-1 border border-gray-300 rounded-md p-2 text-sm bg-gray-50">
                    @endif
                </div>
                @endforeach
            </div>
        ...
        <section class="mt-10 border-t pt-6">
            ...
            <form action="{{ route('admin.ekyc.verify', $data->id) }}" method="POST"
